...
def main():
    ...
    with open(sys.argv[1], 'r') as json_file:
        ...
        print("    }\n", file=body)
        print("    let bincode_slice: &[u8] =", file=body)
        print("        unsafe { std::slice::from_raw_parts(in_bincode_ptr, in_bincode_sz as usize) };\n", file=body)
        ...
        for entry in types_dict:
            ...
            if entry['name'] not in blacklist:
                ...
                if idx != -1:
                    ...
                    print("                std::ptr::copy_nonoverlapping(", file=body)
                    print("                    &ser_sz as *const usize as *const u64,", file=body)
                    print("                    out_ptr as *mut u64,", file=body)
                    print("                    1,", file=body)
                    print("                );", file=body)
                    print("                std::ptr::copy_nonoverlapping(", file=body)
                    print("                    ser.output.as_ptr(),", file=body)
                    print("                    out_ptr.add(std::mem::size_of::<u64>()),", file=body)
                    print("                    ser_sz,", file=body)
                    print("                );", file=body)
                    print("                std::ptr::copy_nonoverlapping(", file=body)
                    print("                    yaml_str.as_ptr(),", file=body)
                    print("                    out_ptr.add(std::mem::size_of::<u64>() + ser_sz),", file=body)
                    print("                    yaml_sz,", file=body)
                    print("                );", file=body)
                    print("            }", file=body)
                    print("        }", file=body)
                else:
