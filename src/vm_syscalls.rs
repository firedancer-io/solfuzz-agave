...
pub fn execute_vm_syscall(input: SyscallContext) -> Option<SyscallEffects> {
    ...
    let txn_ctx = transaction_context.clone();
    ...
    println!("INSTR ACCOUNTS LEN {:?}", instr_accounts.len());
    ...
    let direct_mapping = invoke_context.get_feature_set().is_active(&bpf_account_data_direct_mapping::id());
    ...
    println!("NUM PROGRAM ACCS {:?}", caller_instr_ctx.get_number_of_program_accounts() );

    println!("NUM PROGRAM ACCS {:?}", caller_instr_ctx.get_number_of_program_accounts() );

    let (_aligned_memory, memory_regions, acc_metadatas) = serialize_parameters( &txn_ctx, caller_instr_ctx, !direct_mapping ).unwrap();
    ...
    let sbpf_version: SBPFVersion = SBPFVersion::V0;
    ...
    let regions = regions_without_input.into_iter().chain(memory_regions).collect();
