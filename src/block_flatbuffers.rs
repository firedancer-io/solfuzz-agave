...
fn build_latest_stake_delegations<'ctx>(
    block_context: &block_generated::BlockContext<'ctx>,
    epoch: Epoch,
    ...
    block_context
        ...
        .for_each(|account_state| {
            let pubkey = Pubkey::from(account_state.address());
...
fn build_prev_stake_delegations<'a>(
    vote_accounts: &flatbuffers::Vector<
        'a,
        flatbuffers::ForwardsUOffset<context_generated::VoteAccount<'a>>,
    >,
) -> Stakes<stake_account::StakeAccount<Delegation>> {
    let mut stakes = Stakes::<Delegation>::default();
    vote_accounts.iter().for_each(|input_vote_account| {
        let (pubkey, account) = (&input_vote_account.vote_account()).into();
        let account_shared_data = AccountSharedData::from(account);

        if let Ok(vote_account) = VoteAccount::try_from(account_shared_data) {
            stakes
                .vote_accounts
                .insert(pubkey, vote_account, || input_vote_account.stake());
        }
    });
...
fn get_changed_accounts<'ctx>(
    bank: &Bank,
    context: &block_generated::BlockContext<'ctx>,
) -> Vec<(Pubkey, AccountSharedData)> {
    let mut changed_accounts = Vec::new();

    for initial_account in context.account_states().iter() {
        let pubkey = Pubkey::from(initial_account.address());
        let initial_account_data = AccountSharedData::from(&initial_account);

        if let Some(current_account_data) = bank.get_account(&pubkey) {
            if !solana_account::accounts_equal(&initial_account_data, &current_account_data) {
...
fn create_changed_accounts_bank_hash_details<'ctx>(
    bank: &Bank,
    context: &block_generated::BlockContext<'ctx>,
    ...
    let changed_accounts = get_changed_accounts(bank, context);
...
#[allow(deprecated)]
pub fn execute_block<'ctx, 'buf>(
    context: &block_generated::BlockContext<'ctx>,
    builder: &mut flatbuffers::FlatBufferBuilder<'buf>,
) {
    let bank_fields = context.bank_fields();
    let feature_set: FeatureSet = if let Some(features) = context.features() {
    ...
    let recent_blockhashes: RecentBlockhashes =
        get_sysvar(&sysvar_accounts, &solana_sysvar::recent_blockhashes::id());
    let stake_history: StakeHistory = get_sysvar(&sysvar_accounts, &stake_history::id());

    let current_slot = bank_fields.current_slot();
    let parent_slot = bank_fields.parent_slot();
    let poh = Hash::new_from_array(bank_fields.poh().0);

    let genesis_config = GenesisConfig {
        creation_time: bank_fields.genesis_creation_time() as i64,
        inflation: bank_fields.inflation().into(),
    ...
    context.blockhash_queue().iter().for_each(|blockhash| {
        let blockhash_hash = Hash::new_from_array(blockhash.0);
    ...
    let accounts_to_store = context
        ...
        .map(|account| {
            ...
            (account.address().into(), account_data)
        })
    ...
    let stakes_t = build_latest_stake_delegations(&context, parent_epoch, &stake_history);
    let stakes_t_1 = build_prev_stake_delegations(&bank_fields.vote_accounts_t_1());
    let stakes_t_2 = build_prev_stake_delegations(&bank_fields.vote_accounts_t_2());
    ...
    let stakes_current_accounts = Stakes::new(&stakes_t, |pubkey| {
        context
            .account_states()
            .iter()
            .find(|acct| Pubkey::from(acct.address()) == *pubkey)
            .map(|account| AccountSharedData::from(&account))
    })
    .unwrap();
    epoch_stakes.insert(
    ...
    let parent_lthash = LtHash(bank_fields.parent_lthash().hash().into());
    ...
    let bank_fields = BankFieldsToDeserialize {
        ...
        ancestors,
        hash: Hash::default(),
        parent_hash: bank_fields.parent_bank_hash().into(),
        parent_slot,
        hard_forks: HardForks::default(),
        transaction_count: 0,
        hashes_per_tick: None,
        capitalization: bank_fields.prev_epoch_capitalization(),
        signature_count: 0,
        ...
        genesis_creation_time: bank_fields.genesis_creation_time() as i64,
        ...
        fee_rate_governor: FeeRateGovernor::new_derived(
            ...
            bank_fields.parent_signature_count(),
        ),
    ...
    entries.extend(
        context
            ...
            .iter()
            .map(|txn| {
                let transaction = build_versioned_transaction(&txn);
                Entry {
                    num_hashes: 1u64,
                    hash: Hash::default(),
    ...
    let leader_schedule_effects =
        if let Some(schedule) = leader_schedule.get_epoch_leader_schedule(leader_schedule_epoch) {
            ...
            Some(block_generated::LeaderScheduleEffects::create(
                builder,
                &block_generated::LeaderScheduleEffectsArgs {
                    ...
                    leader_schedule_hash: Some(&block_generated::LeaderScheduleHash::new(
                        &schedule_hash,
                    )), // 128-bit fingerprint of the schedule
                },
            ))
        } else {
            // No schedule found for this epoch
            // This can happen during bootstrapping or if the epoch is too far in the future
            None
        };

    // Then include in the output
    let cost_tracker_effects = block_generated::CostTracker::create(
        builder,
        &block_generated::CostTrackerArgs {
            block_cost: cost_tracker.block_cost(),
            vote_cost: cost_tracker.vote_cost(),
        },
    );

    let block_effects = block_generated::BlockEffects::create(
        builder,
        &block_generated::BlockEffectsArgs {
            has_err: result.is_err(),
            slot_capitalization: no_schedule_bank.capitalization(),
            bank_hash: Some(&context_generated::Hash::new(
                no_schedule_bank.hash().as_bytes(),
            )),
            cost_tracker: Some(cost_tracker_effects),
            leader_schedule: leader_schedule_effects,
        },
    );

    builder.finish_minimal(block_effects);
}
