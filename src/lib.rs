...
impl TryFrom<proto::InstrContext> for InstrContext {
    ...
    fn try_from(input: proto::InstrContext) -> Result<Self, Self::Error> {
        ...
        let mut feature_set: FeatureSet = input
            .epoch_context
            .as_ref()
        ...
        feature_set.activate(&migrate_address_lookup_table_program_to_core_bpf::id(), 0);
        feature_set.activate(&curve25519_syscall_enabled::id(), 0);
        feature_set.activate(&consume_blockstore_duplicate_proofs::id(), 0);
        feature_set.activate(&get_sysvar_syscall_enabled::id(), 0);
        feature_set.activate(&migrate_feature_gate_program_to_core_bpf::id(), 0);
        feature_set.activate(&move_precompile_verification_to_svm::id(), 0);
        feature_set.activate(&enable_transaction_loading_failure_fees::id(), 0);
        feature_set.activate(&reserve_minimal_cus_for_builtin_instructions::id(), 0);
