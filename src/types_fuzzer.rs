...
pub unsafe extern "C" fn sol_compat_type_execute_v1(
    ...
    let bincode_slice: &[u8] =
        unsafe { std::slice::from_raw_parts(in_bincode_ptr, in_bincode_sz as usize) };
    ...
    match bincode_slice[0] {
            ...
            unsafe {
                *out_psz = (std::mem::size_of::<u64>() + ser_sz + yaml_sz) as u64;
                std::ptr::copy_nonoverlapping(
                    &ser_sz as *const usize as *const u64,
                    out_ptr as *mut u64,
                    1,
                );
                std::ptr::copy_nonoverlapping(
                    ser.output.as_ptr(),
                    out_ptr.add(std::mem::size_of::<u64>()),
                    ser_sz,
                );
                std::ptr::copy_nonoverlapping(
                    yaml_str.as_ptr(),
                    out_ptr.add(std::mem::size_of::<u64>() + ser_sz),
                    yaml_sz,
                );
            }
        }
        ...
        33 => {
            let typ: BankIncrementalSnapshotPersistence =
                if let Ok(h) = bincode::deserialize(&bincode_slice[1..]) {
                    h
                } else {
                    return 0;
                };
